...
import { 
  PlusCircle, Users as UsersIcon, 
  Video, FileText, CloudUpload, Trash2, Edit3, 
  XCircle, CheckCircle, ShieldCheck, Database, Settings,
  Search, SlidersHorizontal, Loader2, ListTree, Target,
  Activity, ShieldAlert, BarChart3, Globe
} from "lucide-react";
...
export default function Admin() {
  ...
  const handleRemoveResource = async (index) => {
    ...
    if (resourceToDelete?.fileUrl?.includes("cloudinary.com")) {
      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/upload/delete`, { url: resourceToDelete.fileUrl }, { headers: { clerkid: user.id } });
      } catch (err) { console.error("Cleanup Failed"); }
    }
  ...
  if (!isLoaded) return <div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="text-orange-500 animate-spin" size={40} /></div>;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
          ...
          <nav className="flex gap-2 bg-gray-900/50 p-1.5 rounded-2xl border border-gray-800">
            {["create", "courses", "users"].map((t) => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === t ? "bg-orange-500 text-black shadow-lg shadow-orange-500/20" : "text-gray-500 hover:text-white"}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          <div className="bg-gray-900/20 border border-gray-800 p-6 rounded-[2rem] group hover:border-orange-500/50 transition-all">
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Population</p>
            <h3 className="text-3xl font-black italic mt-1">{stats.totalUsers}</h3>
          </div>
          <div className="bg-gray-900/20 border border-gray-800 p-6 rounded-[2rem] group hover:border-blue-500/50 transition-all">
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Admins</p>
          ...
          <div className="bg-gray-900/20 border border-gray-800 p-6 rounded-[2rem] group hover:border-purple-500/50 transition-all">
            ...
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Modules</p>
        ...
        {activeTab !== "create" && (
          <div className="mb-8 relative max-w-xl group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-orange-500 transition-colors" size={18} />
            <input 
              type="text" 
              placeholder={activeTab === "courses" ? "Search DB entries..." : "Search user directory..."}
        ...
        {activeTab === "create" && (
          <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-gray-900/20 border border-gray-800 p-6 md:p-10 rounded-[3rem]">
              <h2 className="text-xl font-bold italic mb-8 flex items-center gap-2"><Settings size={20} className="text-orange-500" /> System Config</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <input type="text" placeholder="YouTube URL" className="admin-input-v2" value={course.videoUrl} onChange={(e) => setCourse({...course, videoUrl: e.target.value})} required />
                 ...
                 <select className="admin-input-v2 md:col-span-2" value={course.category} onChange={(e) => setCourse({...course, category: e.target.value})}>
                    <option value="Tech">General Tech</option>
                    <option value="Python">Python</option>
                    <option value="React">React</option>
                    <option value="Backend">Backend</option>
                 </select>
                 <textarea placeholder="Description" className="admin-input-v2 h-32 py-4 md:col-span-2" value={course.description} onChange={(e) => setCourse({...course, description: e.target.value})} required />
              </div>
            </div>
            ...
            <div className="bg-gray-900/20 border border-gray-800 p-6 md:p-10 rounded-[3rem]">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-sm font-black text-orange-500 uppercase tracking-widest italic flex items-center gap-2"><ListTree size={18}/> Curriculum</h2>
                <button type="button" onClick={() => addField('playlist')} className="bg-white text-black px-4 py-2 rounded-xl text-[10px] font-black uppercase">+ Add Lesson</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(course.playlist || []).map((item, idx) => (
                  <div key={idx} className="relative bg-black/40 p-5 rounded-[1.5rem] border border-gray-800 group">
                    <input placeholder="Lesson Title" className="bg-transparent w-full text-sm font-bold outline-none border-b border-gray-800 mb-3" value={item.title} onChange={(e) => handleArrayChange(idx, 'playlist', 'title', e.target.value)} />
                    <input placeholder="YT Video URL" className="bg-transparent w-full text-[10px] text-orange-500/50 outline-none" value={item.videoUrl} onChange={(e) => handleArrayChange(idx, 'playlist', 'videoUrl', e.target.value)} />
            ...
            <div className="bg-gray-900/20 border border-gray-800 p-6 md:p-10 rounded-[3rem]">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-sm font-black text-orange-500 uppercase tracking-widest italic flex items-center gap-2"><Target size={18}/> Mission Tasks</h2>
                <button type="button" onClick={() => addField('assignments')} className="bg-white text-black px-4 py-2 rounded-xl text-[10px] font-black uppercase">+ New Task</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(course.assignments || []).map((a, i) => (
                  <div key={i} className="relative bg-black/40 p-5 rounded-[1.5rem] border border-gray-800 group">
                    <input placeholder="Task Name" className="bg-transparent w-full text-sm font-bold outline-none border-b border-gray-800 mb-3" value={a.title} onChange={(e) => handleArrayChange(i, 'assignments', 'title', e.target.value)} />
                    <input placeholder="Link" className="bg-transparent w-full text-[10px] text-orange-500/50 outline-none" value={a.fileUrl} onChange={(e) => handleArrayChange(i, 'assignments', 'fileUrl', e.target.value)} />
            ...
            <div className="bg-gray-900/20 border border-gray-800 p-6 md:p-10 rounded-[3rem]">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-sm font-black text-orange-500 uppercase tracking-widest italic flex items-center gap-2"><FileText size={18}/> Asset Vault</h2>
                <button type="button" onClick={() => addField('resources')} className="bg-white text-black px-4 py-2 rounded-xl text-[10px] font-black uppercase">+ Sync Asset</button>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {(course.resources || []).map((r, i) => (
                  <div key={i} className="flex flex-col gap-4 p-6 bg-black/40 border border-gray-800 rounded-[2rem] relative group border-l-4 border-l-orange-500">
                    <div className="flex gap-2">
                        ...
                        <input placeholder="Name" className="admin-input-v2 !py-2 !text-[11px] w-2/3" value={r.title} onChange={(e) => handleArrayChange(i, 'resources', 'title', e.target.value)} />
                    </div>
                    <div className="flex items-center gap-3">
                        <input type="file" className="hidden" id={`vault-${i}`} onChange={(e) => handleFileChange(e, i)} />
                        <label htmlFor={`vault-${i}`} className="flex-1 text-center p-3 bg-gray-900 border border-dashed border-gray-700 rounded-xl text-[10px] cursor-pointer hover:border-white transition-all text-gray-500 font-bold truncate">
                            {selectedFiles[i] ? selectedFiles[i].name : "Select Device File"}
                        </label>
                        {selectedFiles[i] && (
                            <button type="button" onClick={() => uploadToCloudinary(i)} className="bg-orange-500 text-black p-3 rounded-xl hover:bg-white transition-all">
                                {fileUploading === i ? <Loader2 className="animate-spin" size={18}/> : <CloudUpload size={18}/>}
                            </button>
                        )}
                    </div>
                    {r.fileUrl && <p className="text-[9px] text-green-500 font-bold uppercase italic ml-1 tracking-widest">✅ Link Secured</p>}
                    <button type="button" onClick={() => removeField(i, 'resources')} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-xl">✕</button>
                  </div>
                ))}
              </div>
            </div>

            <button type="submit" disabled={loading} className="w-full bg-orange-500 text-black py-6 rounded-[2rem] font-black uppercase text-sm tracking-[0.3em] shadow-2xl hover:bg-white transition-all active:scale-[0.98]">
              {loading ? "Decrypting Buffer..." : isEditing ? "Push Changes" : "Deploy Entry"}
            </button>
          </form>
        )}
        ...
        {activeTab === "courses" && (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-bottom-4 duration-500">
              {filteredCourses.map(c => (
                <div key={c._id} className="bg-gray-900/20 border border-gray-800 p-5 rounded-[2.5rem] flex items-center justify-between group border-l-4 border-l-orange-500 hover:bg-gray-900/40 transition-all">
                  <div className="flex items-center gap-5 truncate">
                    <img src={c.thumbnail} className="w-20 md:w-28 h-14 object-cover rounded-2xl border border-gray-800" />
                    <div className="truncate">
                      <h3 className="font-bold text-sm md:text-base truncate group-hover:text-orange-500 transition-colors">{c.title}</h3>
                      <p className="text-[10px] text-gray-500 uppercase font-black">{c.instructor}</p>
                    </div>
                  </div>
                    ...
                    {filteredUsers.map(u => (
                      <tr key={u.clerkId} className="hover:bg-white/[0.01] transition-colors group">
                        <td className="p-8 font-bold text-sm">
                          <div>{u.name}</div>
                          <div className="text-[10px] text-gray-600 lowercase font-normal">{u.email}</div>
                        </td>
                        <td className="p-8">
                        ...
                        <td className="p-8">
                          <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 rounded-full ${u.isBlocked ? 'bg-red-500 shadow-[0_0_5px_red]' : 'bg-green-500 shadow-[0_0_5px_green]'}`}></div>
                            <span className="text-[10px] font-bold text-gray-500 uppercase">{u.isBlocked ? "Blocked" : "Active"}</span>
                          </div>
                        </td>
                        <td className="p-8 text-right">
                          <button 
                            onClick={() => handleToggleBlock(u.clerkId, u.isBlocked)} 
